...
export type ActionList = {
  ...
  functions: {
    [key: string]:
      | (() => void | true)
      | { function: () => void | true; disabled: () => boolean };
  };
};
...
export default function (
  ...
  camera.rotation.x = 1;
  const cameraStartedQuaternion = new THREE.Quaternion().copy(
    camera.quaternion
  );
  ...
  const ActionSets: { default: ActionList; raising: ActionList } = {
    default: {
      functions: {
        ...
        Raise: {
          disabled: () =>
            ActionSets.raising.slider
              ? ActionSets.raising.slider.min > ActionSets.raising.slider.max
              : true,
          function: () => {
            if (!ActionSets.raising.slider)
              throw Error("ActionSets.raising.slider is null");
  ...
  function _createGround() {
    ...
    dracoLoader.setDecoderPath(
      "https://cdn.jsdelivr.net/gh/mrdoob/three.js/examples/jsm/libs/draco/"
    );
    loader.setDRACOLoader(dracoLoader);

    loader.load(
      "./compressed_1738430775587_untitled2.glb",
      function (gltf) {
  ...
  function createLights() {
    ...
    const spotLight = new THREE.SpotLight(0xffa500, 5);
  ...
  function _documentEvents() {
    window.onmousemove = (event) => {
      ...
      camRotation.y =
        Math.PI - ((clamp(mousePrecent.y, 0.3, 0.75) + 0.75) / 2) * Math.PI;
      camRotation.x = (0.5 - mousePrecent.x) * multiplyer * 1.2;
      ...
      Promise.all([f1, f2])
        .then((v) => {
          ...
          const sprite = new THREE.Mesh(
            new THREE.BoxGeometry(0.58 / 2, 0.78 / 2, 0.0),
            material1
          );
          const spriteA = new THREE.Mesh(
            new THREE.BoxGeometry(0.58 / 2, 0.78 / 2, 0.0),
            material2
          );
          const behind1 = new THREE.Vector3()
            ...
            .add(
              new THREE.Vector3(0.1, -0.5, -1).applyQuaternion(
                cameraStartedQuaternion
              )
            );
          const behind2 = new THREE.Vector3()
            .copy(camera.position)
            .add(
              new THREE.Vector3(-0.1, -0.5, -1.02).applyQuaternion(
                cameraStartedQuaternion
              )
            );
          ...
          const after1 = new THREE.Vector3()
            .copy(camera.position)
            .add(
              new THREE.Vector3(0.1, -0.475, -0.95).applyQuaternion(
                cameraStartedQuaternion
              )
            );
  ...
  function _postProcessing() {
    ...
    outlinePassCards = new OutlinePass(
    ...
    outlinePassBodies = new OutlinePass(
      new THREE.Vector2(window.innerWidth, window.innerHeight),
      scene,
      camera
    );
  ...
  function _socketInitiate() {
      ...
      if (onlinesmeshes.length >= 2)
        throw new Error("too many", {
          cause: {
            ...
            whatSize: Array.from(clients.values()).filter(
              (v) => v.myturn === true
            ).length,
          },
        });
    ...
    function orderOnlines() {
      let theta = -Math.PI;
      for (const player of Array.from(clients.values()).sort(
        (a, b) => a.order - b.order
      )) {
    ...
    socket.on(
      "i",
      (args: { op: { [k: string]: { n: string; o: number } } }) => {
    ...
    socket.on(
      "st",
      (args: { b: number[]; c1: number; c2: number; rn: number }) => {
    ...
    socket.on("act", (args: { m: number; id: string }) => {
      ...
      if (args.id !== socket.id) {
        ...
        ActionSets.raising.slider
          ? (ActionSets.raising.slider.min =
              ActionSets.raising.slider.defaultValue =
                args.m + 50)
          : 0;
        react.SetAction(ActionSets.default);
      }
    ...
    socket.on("bl", (args: { id: string; b: number; c: number; s: number }) => {
      ...
      if (args.id === socket.id) {
        ...
        ActionSets.raising.slider
          ? (ActionSets.raising.slider.max = localPlayer.money)
          : 0;
      } else {
    ...
    socket.on("win", (args: { [key: string]: number }) => {
      // alert(str);
      // console.log("win 1");
      const winnerId = Object.keys(args).reduce((a, b) =>
        args[a] > args[b] ? a : b
      );
      // console.log("win 2");
      ...
      react.setWinName(
        clientIsWinner ? "YOU WON" : `${clients.get(winnerId)?.name} WIN`
      );
      setTimeout(() => {
  ...
  renderer.setAnimationLoop(() => {
    ...
    localPlayer.update();
    for (const ikUpdate of Array.from(clients.values()).map((v) => v.update))
      ikUpdate(camera);
